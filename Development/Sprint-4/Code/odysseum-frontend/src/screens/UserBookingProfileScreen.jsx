...
const getQueryBooking = async ({bookingId}) => {
  try {
    const res = await axiosInstance.get(`/booking/getById?bookingId=${bookingId}`)
...
const UserBookingProfileScreen = ({bookingId}) => {
  ...
  const [cancelReasonModalVisible, setCancelReasonModalVisible] = React.useState(false)
  const [transactionModalVisible, setTransactionModalVisible] = React.useState(false)
  ...
  const getStatusBgColor = (status) => {
    switch(status) {
      case 'confirmed': return 'bg-emerald-900/30'
      case 'pending': return 'bg-amber-900/30'
      case 'cancelled': return 'bg-rose-900/30'
  ...
  const handleCancelBooking = () => {
    ...
    axiosInstance.post('/booking/cancel', { bookingId: bookingId, userId: user._id, reason: cancelReason })
    .then(res => {
      // console.log(res.data)
  ...
  if(error)
  {
    return (
      <View className="bg-[#070f1b] flex-1 justify-center items-center">
        ...
        <Text className="text-gray-400 mb-4">Unable to fetch booking details.</Text>
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#070f1b]">
      <View className="flex-row items-center mt-4 px-5 gap-x-6">
        ...
        <View>
          <Text className="font-dsregular text-gray-400 text-base">Booking</Text>
          <Text className="font-dsbold text-white text-2xl mt-1">{booking?.serviceId.name}</Text>
        </View>
      </View>
      ...
      <ScrollView className="flex-1 mt-5" showsVerticalScrollIndicator={false}>
        <View className="py-2 px-5">
          {/* Status Card */}
          <View className={`${getStatusBgColor(booking.status)} rounded-2xl p-4 mb-6 border border-gray-700`}>
            <View className="flex-row items-center justify-between">
              <View className="flex-row items-center gap-x-3">
                <View className={`${getStatusColor(booking.status)} p-2 rounded-full`}>
              ...
              <View className="bg-gray-800/70 px-3 py-1 rounded-lg">
                <Text className="text-white font-medium"># {bookingId?.slice(-6) || '123456'}</Text>
              </View>
            </View>
          </View>
          ...
          <View className="bg-gray-800/50 rounded-2xl p-6 mb-6 border border-gray-700">
            <Text className="text-white font-dsbold text-lg mb-4">Booking Details</Text>
            <BookingDetailRow 
            ...
            <BookingDetailRow 
              ...
              label="Time Slot"
              value={`${new Date(booking.timeSlot.startTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} - ${new Date(booking.timeSlot.endTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}`}
            />
            <BookingDetailRow 
          ...
          {/* Pricing Details */}
          <View className="bg-gray-800/50 rounded-2xl p-6 mb-6 border border-gray-700">
            <Text className="text-white font-dsbold text-lg mb-4">Payment Details</Text>
            <BookingDetailRow 
              label="Base Price"
            ...
            {booking.pricing.specialPrice.applied && (
              <BookingDetailRow 
                ...
                value={`-$${booking.pricing.basePrice - booking.pricing.specialPrice.price}`}
          ...
          {/* Payment Status */}
          <View className="bg-gray-800/50 rounded-2xl p-6 mb-6 border border-gray-700">
            <Text className="text-white font-dsbold text-lg mb-4">Payment Status</Text>
            <View className="flex-row items-center justify-between bg-gray-700/50 p-3 rounded-xl">
              <View className="flex-row items-center">
                <View className={booking.payment.status === 'fully_paid' ? "bg-emerald-500/20 p-2 rounded-full mr-3" : 
                              booking.payment.status === 'deposit_paid' ? "bg-amber-500/20 p-2 rounded-full mr-3" :
                ...
                <Text className="text-white font-medium capitalize">{booking.payment.status.replace('_', ' ')}</Text>
              </View>
              <TouchableOpacity
            ...
            {booking.status === 'confirmed' && (booking.payment.status === 'deposit_paid' || booking.payment.status === 'pending') && (
              <TouchableOpacity 
                className="bg-indigo-500 rounded-xl p-4 flex-row items-center justify-center mt-4"
                onPress={() => router.push(`/payment-completion/${bookingId}`)}
              >
          ...
          {booking.status === 'cancelled' && (
            <View className="bg-gray-800/50 rounded-2xl p-6 mb-6 border border-gray-700">
              <Text className="text-white font-dsbold text-lg mb-4">Cancellation Details</Text>
              <View className="bg-rose-500/10 p-4 rounded-xl border border-rose-500/30 mb-4">
                <View className="flex-row items-start">
                  ...
                  <Text className="text-rose-300 ml-2 flex-1">This booking has been cancelled</Text>
                </View>
              </View>
              <BookingDetailRow 
                label="Reason"
              ...
              <BookingDetailRow 
                label="Cancelled On"
                value={booking.cancellation.cancellationDate ? new Date(booking.cancellationDate).toLocaleString() : new Date().toLocaleString()}
              />
              <BookingDetailRow 
                label="Refund Amount"
              ...
              <BookingDetailRow 
                ...
                valueColor={booking.cancellation.cancellationFee > 0 ? "text-rose-400" : "text-white"}
                isLast
              />
            </View>
          )}
          ...
          {booking.serviceId.cancellationPolicy.allowCancellation && booking.status !== 'cancelled' && (
            <View className="mb-8">
              <View className="bg-gray-800/50 rounded-2xl p-6 border border-gray-700">
                <Text className="text-white font-dsbold text-lg mb-4">Cancellation Policy</Text>
                <View className="bg-indigo-500/10 p-4 rounded-xl border border-indigo-500/30 mb-4">
                  <View className="flex-row items-start">
                    ...
                    <Text className="text-gray-300 ml-2 flex-1">
                      ...
                      {new Date(new Date(booking.bookingDate).getTime() - booking.serviceId.cancellationPolicy.freeCancellationHours * 3600000)
                        .toLocaleString([], {
                          month: 'short',
                          day: 'numeric',
                          hour: '2-digit',
      ...
      >
        <View className="flex-1 justify-end bg-black/50">
          <View className="bg-[#0f172a] rounded-t-3xl p-6">
            <Text className="text-white font-dsbold text-xl mb-4">Cancel Booking</Text>
            <Text className="text-gray-400 mb-4">Please provide a reason for cancellation:</Text>
      ...
      >
        <View className="flex-1 justify-end bg-black/50">
          <View className="bg-[#0f172a] rounded-t-3xl p-6">
            <Text className="text-white font-dsbold text-xl mb-4">Payment Transactions</Text>
            ...
            {booking.payment.transactions && booking.payment.transactions.length > 0 ? (
                ...
                {booking.payment.transactions.map((transaction, index) => (
                  <View key={index} className="bg-gray-800 rounded-xl p-4 mb-3">
                    <View className="flex-row justify-between mb-2">
                      ...
                      <Text className="text-white capitalize">{transaction.transactionType}</Text>
                    </View>
                    <View className="flex-row justify-between mb-2">
                    ...
                    <View className="flex-row justify-between mb-2">
                      ...
                      <Text className="text-white">{new Date(transaction.date).toLocaleString()}</Text>
                    </View>
                    <View className="flex-row justify-between mb-2">
                    ...
                    <View className="flex-row justify-between">
                      ...
                      <Text className="text-white">{transaction.paymentMethod}</Text>
                    </View>
                    {transaction.transactionId && (
                      <View className="flex-row justify-between mt-2 pt-2 border-t border-gray-700">
                        ...
                        <Text className="text-gray-300 text-xs">{transaction.transactionId}</Text>
                      </View>
                    )}
                  </View>
                ))}
              ...
              <View className="bg-gray-800 rounded-xl p-4 mb-4 items-center">
                <Text className="text-gray-400">No transaction records found</Text>
              </View>
            )}
...
const BookingDetailRow = ({ icon, label, value, bold = false, valueColor = "text-white", isLast = false }) => (
  <View className={`flex-row items-center ${isLast ? '' : 'mb-4'}`}>
    {icon && <View className="mr-3 w-6">{icon}</View>}
    <View className="flex-1">
      <Text className="text-gray-400">{label}</Text>
    </View>
    <Text className={`${valueColor} ${bold ? 'font-dsbold text-lg' : 'font-medium'}`}>
      {value}
    </Text>
  </View>
)
