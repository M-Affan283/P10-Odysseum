...
def get_reviews_by_business(business_id):
    ...
    reviews = collection.find({"entityId": ObjectId(business_id)}).sort([("createdAt", -1)]).limit(20)
    return reviews

def get_reviews_by_location(location_id):
    db = connect_to_db()
    collection = db['Review']
    reviews = collection.find({"entityId": ObjectId(location_id)}).sort([("createdAt", -1)]).limit(20)
    return reviews
