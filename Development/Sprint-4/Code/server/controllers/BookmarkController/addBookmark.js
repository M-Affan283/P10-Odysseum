  ...
  try {
    ...
    if (user.bookmarks.includes(locationId)) {
      ...
      user.bookmarks = user.bookmarks.filter(
        (bookmark) => bookmark != locationId
      );
    } else {
      console.log("Adding bookmark...");
      user.bookmarks.push(locationId);
      updatedEntity = await entityMetricUpdator(
        "Location",
        locationId,
...
export const bookMarkBusiness = async (req, res) => {
  ...
  try {
    ...
    // Check if business is already bookmarked. otherwise add it to bookmarks
    if (user.businessBookmarks.includes(businessId)) {
      console.log("Removing bookmark...");
      user.businessBookmarks = user.businessBookmarks.filter(
        (bookmark) => bookmark != businessId
      );

      // updatedEntity = entityMetricUpdator("Business", businessId, InteractionTypes.UNBOOKMARK);
    } else {
      console.log("Adding bookmark...");
    ...
    return res.status(200).json({
      message: "Business bookmarks updated successfully",
    });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
      .json({ message: "Error updating business bookmark" });
  }
};
