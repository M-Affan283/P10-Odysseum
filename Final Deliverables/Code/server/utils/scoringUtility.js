...
const calculateHeatmapScore = (activityCount, avgRating, lastInteraction) => {
  ...
  const normalizedActivity =
    Math.log10(activityCount /*+ 1*/) / Math.log10(100); // Assumes 100 is a high activity count
...
export const entityMetricUpdator = async (
  entityType,
  entityId,
  interactionType,
  rating = null
  ...
  try {
    ...
    // Get the correct rating property based on entity type
    const ratingProperty =
      entityType === "Location" ? "avgRating" : "averageRating";
    let newRating = entity[ratingProperty] || 0;
    ...
    if (interactionType === InteractionTypes.REVIEW && rating !== null) {
      ...
      const calculatedAvgRating =
        entityReviews.reduce((acc, review) => acc + review.rating, 0) /
        entityReviews.length;
      newRating = calculatedAvgRating;
    }

    // Calculate new heatmap score
    const newHeatmapScore = calculateHeatmapScore(
      entity.activityCount + InteractionWeights[interactionType],
      newRating,
      now
    );

    // Create update object with appropriate property name
    const updateObj = {
      $inc: { activityCount: InteractionWeights[interactionType] },
      $set: {
        lastInteraction: now,
        heatmapScore: newHeatmapScore,
      },
    };

    // Set the correct rating property in the update object
    updateObj.$set[ratingProperty] = newRating;
    ...
    if (entityType === "Business") {
        console.log("locationid: ", entity.locationId)
      const location = await Location.findById(entity.locationId);

      if (!location) throw new Error("Location not found");

      // Use the correct property name for location's rating
      const locationRating = location.avgRating || 0;

      const newLocationHeatmapScore = calculateHeatmapScore(
        location.activityCount + InteractionWeights[interactionType] * 0.5,
        locationRating,
        now
      );

      // Validate the score is a valid number before updating
      const validHeatmapScore = isNaN(newLocationHeatmapScore)
        ? 0
        : newLocationHeatmapScore;

      console.log("Location heatmap score: ", validHeatmapScore);

      await Location.findByIdAndUpdate(
        entity.locationId,
        {
          $inc: { activityCount: InteractionWeights[interactionType] * 0.5 },
          $set: { lastInteraction: now, heatmapScore: validHeatmapScore },
        },
        { new: true, runValidators: true }
      );
      // }, { new: true, runValidators: true, session });
    ...
    throw error;
    // console.log("Error in entityMetricUpdator: ", error);
  }
  // finally
  // {
  //     session.endSession();
